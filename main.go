...
func Connect() {
	...
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS Nurse_appointment(Bookingid INT NOT NULL AUTO_INCREMENT,Patient_id INT,Nurse_id INT,Booking_time VARCHAR(10),PRIMARY KEY (Bookingid),FOREIGN KEY (Patient_id) REFERENCES Patient(ID),FOREIGN KEY (Doctor_id) REFERENCES Nurse(ID));")
	...
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS Doctor_feedback(feedbackid INT NOT NULL AUTO_INCREMENT,Patient_id INT,Doctor_id INT,Rating INT,Feedback_msg VARCHAR(255),PRIMARY KEY (feedbackid),FOREIGN KEY (Patient_id) REFERENCES Patient(ID),FOREIGN KEY (Doctor_id) REFERENCES Doctor(ID));")
	...
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS Nurse_feedback(feedbackid INT NOT NULL AUTO_INCREMENT,Patient_id INT,Nurse_id INT,Rating INT,Feedback_msg VARCHAR(255)PRIMARY KEY (feedbackid),FOREIGN KEY (Patient_id) REFERENCES Patient(ID),FOREIGN KEY (Doctor_id) REFERENCES Doctor(ID));")
